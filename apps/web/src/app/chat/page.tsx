...
export default function ChatPage() {
  ...
  const { displayedId, phase } = useConversationTransition(selectedConversation?.id ?? null);
  ...
  const typingUserName = useMemo(() => {
    ...
    const member = displayedConversation.members.find((m) => m.id === typingUserId);
  ...
  const userResults = shouldSearchUsers ? (userSearchQuery.data?.users ?? []) : [];
          ...
          {displayedConversation ? (
            <FeatureBoundary name="ChatPanel">
              <ChatPanel
                ...
                conversationName={getConversationName(displayedConversation, user?.id ?? 0)}
                isTyping={isTyping}
                onOpenSidebar={() => setSidebarOpen(true)}
                otherMemberId={
                  displayedConversation.type === "dm"
                    ? displayedConversation.members.find((m) => m.id !== user?.id)?.id
                    : undefined
                }
              />
            </FeatureBoundary>
