...
import { publishConversationEvent, publishMembershipChange } from "../../services/realtime-events";
...
export function registerRealtimeHandlers(db: Database): void {
  ...
  domainEvents.on("message.read", async ({ conversationId, messageIds, userId }) => {
    await publishConversationEvent(conversationId, {
      type: "message_read",
      conversationId,
      messageIds,
  ...
  domainEvents.on("conversation.created", async ({ conversationId, memberIds }) => {
    for (const memberId of memberIds) {
      const summary = await fetchConversationSummary(db, memberId, conversationId);
      if (summary) {
  ...
  domainEvents.on("conversation.renamed", async ({ conversationId, name }) => {
    await publishConversationEvent(conversationId, {
  ...
  domainEvents.on("member.added", async ({ conversationId, userId }) => {
    const summary = await fetchConversationSummary(db, userId, conversationId);
    if (summary) {
  ...
  domainEvents.on("member.removed", async ({ conversationId, userId }) => {
    await publishConversationEvent(conversationId, {
      type: "member_removed",
      conversationId,
      userId,
